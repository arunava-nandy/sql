...
select bidder_id,(sum((case when bid_status='won' then 1 else 0 end))/count(*))*100 as'percent of win' from ipl_bidding_details group by bidder_id;

-- 2.	Display the number of matches conducted at each stadium with stadium name, city from the database.

select ipl_stadium.stadium_name,city,count(*) from ipl_match_schedule inner join ipl_stadium on ipl_match_schedule.stadium_id=ipl_stadium.stadium_id 
group by ipl_stadium.stadium_id;

-- 3.	In a given stadium, what is the percentage of wins by a team which has won the toss?
select ist.stadium_name,(sum((case when toss_winner=match_winner then 1 else 0 end))/count(*))*100 as 'percentage of win' 
...
select bid_team,team_name,count(*) from ipl_bidding_details inner join ipl_team on ipl_bidding_details.bid_team=ipl_team.team_id group by bid_team order by bid_team; 

-- 5.	Show the team id who won the match as per the win details.

select case when match_winner=1 then team_id1
else team_id2 end as match_winner ,win_details from ipl_match ;

-- 6.	Display total matches played, total matches won and total matches lost by team along with its team name.

select ipl_team_standings.team_id,team_name,sum(matches_played)total_matches,sum(matches_won)total_win,sum(matches_lost)total_loss from ipl_team_standings inner join ipL_Team 
on ipl_team_standings.team_ID=ipl_team.team_Id group by ipl_team_standings.team_id;

-- 7.	Display the bowlers for Mumbai Indians team.
use ipl;
select ipl_team_players.team_id,ipl_team_players.player_id,player_role,team_name,ipl_player.player_name 
from ipl_team_players inner join ipl_team 
on ipl_team_players.team_id=ipl_team.team_id join ipl_player 
on ipl_player.player_id=ipl_team_Players.player_id 
and player_role='bowler' and ipl_team_players.team_id=5;
 
 
-- 8.	How many all-rounders are there in each team, Display the teams with more than 4 
-- all-rounder in descending order.

select ipl_team_players.team_id,team_name,count(*) as 'total all-rounders' from ipl_team_players inner join ipl_team 
on ipl_team_players.team_id=ipl_team.team_Id where player_role='All-Rounder' group by ipl_team_players.team_id having count(*)>4 order by count(*) desc;